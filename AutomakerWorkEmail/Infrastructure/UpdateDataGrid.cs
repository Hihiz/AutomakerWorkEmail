    ...
    internal class UpdateDataGrid
    {
        public void RefreshData()
        {
            using (AutomakerWorkEmailContext db = new AutomakerWorkEmailContext())
            {
                ...
                (trackingClientOrderWindow.FindName("textBlockCountClientOrder") as TextBlock).Text = $"Количество Активных заказов: {db.ClientOrders.Where(s => s.Status != "Выдан").Count()}";
                (trackingClientOrderWindow.FindName("textBlockCountCloseClientOrder") as TextBlock).Text = $"Количество Закрытых заказов: {db.ClientOrders.Where(s => s.Status == "Выдан").Count()}";
            }
        }
    }
}
